...
of length 100 (or less if the array is smaller) and compressed. The
...
the most compact and it will be used for R objects that have at least X
and Y dimensions, identified by the names set on the dimensions, and an
optional third axis which is typically an image band or a discrete
(class) axis – the third axis may not represent height/depth (Z) or time
(T). The coordinates must be numeric and regularly spaced and the Y
coordinates must be decreasing. In other words, the “spatial” convention
will be used for imagery style, north-up arrays with a coordinate system
tied to the top-left corner of the array space. For all other cases the
